...
# --- CSS FOR MODERN UI (YOUR ORIGINAL DESIGN) ---
st.markdown("""
    <style>
...
@st.cache_resource
def load_models():
    # Added memory-efficient loading to prevent crash
    processor = BlipProcessor.from_pretrained("Salesforce/blip-image-captioning-base")
    if os.path.exists(LOCAL_CAP_PATH):
        cap_model = BlipForConditionalGeneration.from_pretrained(LOCAL_CAP_PATH, low_cpu_mem_usage=True)
        vqa_model = BlipForQuestionAnswering.from_pretrained(LOCAL_VQA_PATH, low_cpu_mem_usage=True)
        status = "Using Local Fine-tuned Weights"
    else:
        cap_model = BlipForConditionalGeneration.from_pretrained("Salesforce/blip-image-captioning-base", low_cpu_mem_usage=True)
        vqa_model = BlipForQuestionAnswering.from_pretrained("Salesforce/blip-vqa-base", low_cpu_mem_usage=True)
    ...
    with col1:
        ...
        if st.button("Execute Narrative Engine"):
            with st.spinner("Processing visual tokens..."):
                # Added torch.no_grad() to stop RAM usage from increasing
                with torch.no_grad():
